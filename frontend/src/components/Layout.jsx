...
function Layout({ children }) {
  ...
  const [searchResults, setSearchResults] = useState({ transactions: [], categories: [] });
  ...
  useEffect(() => {
    api.get("/auth/me")
      .then((res) => setProfile({ username: res.data.username || "", email: res.data.email || "" }))
      .catch(() => {});
  }, []);
  ...
  const handleSearch = useCallback((query) => {
    ...
    if (!query.trim()) {
      setSearchResults({ transactions: [], categories: [] });
      setShowSearch(false);
      return;
    }
    setShowSearch(true);
    ...
    searchTimeout.current = setTimeout(async () => {
      try {
        ...
        const filteredTx = (txRes.data || []).filter(
          (t) =>
        ...
        setSearchResults({ transactions: filteredTx, categories: filteredCat });
      } catch {
        setSearchResults({ transactions: [], categories: [] });
      } finally {
          ...
          <label className="search-box" ref={searchBoxRef}>
            ...
            {showSearch && (
              <div className="search-dropdown">
                ...
                ) : searchResults.transactions.length === 0 && searchResults.categories.length === 0 ? (
